...
public class Shape {
    private static final Color SHADOW_COLOR = new Color(230, 158, 60);
    ...
    // EFFECTS: constructs a new Shape with the given Point (topLeft) and the midiSynth
    ...
    // REQUIRES: w, h >= 0
    // EFFECTS: constructs Shape at the given x,y-coordinates, with width w and height h
    ...
    // MODIFIES: this
    // EFFECTS:  sets the playLineCoord field of this Shape to playLineCoord
    public void setPlayLineCoord(int playLineCoord) {
        this.playLineCoord = playLineCoord;
    }

    // REQUIRES: the x-coordinate must be >= this.x and the y-coordinate must be >= this.y
    // MODIFIES: this
    ...
    // MODIFIES: this
    // EFFECTS:  adds dx to this.x and dy to this.y. If the sound associated with the new y-coordinate
    //           is different, play the new sound
